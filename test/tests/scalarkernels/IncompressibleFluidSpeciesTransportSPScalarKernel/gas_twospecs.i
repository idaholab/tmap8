...
[Variables]
  ...
  [Cw1]
    family = SCALAR
    initial_condition = '${fparse ${Cout}*2}'
  []
  [Cw2]
    family = SCALAR
...
[Postprocessors]
  ...
  [Jgas]
    type = ParsedPostprocessor
    expression = '2 * ${recomb} * 2*(${Cout})^2 - ${dissoc} * C / ${fsol}'
    pp_names = 'C'
    execute_on = 'TIMESTEP_END'
  []
  [Jmetal]
  ...
  [Jsolvent]
    type = ParsedPostprocessor
    expression = 'Kt / 1.380649E-23 / 6.022E+23 / T * (2*(${Cout})^2 / (${wsol})^2 * sqrt(${fsol}) - C)'
    pp_names = 'Kt C T'
    execute_on = 'TIMESTEP_END'
  []
  [analytical_Csolvent]
