...
/*                   ALL RIGHTS RESERVED                    */
/************************************************************/
#include "FlowPathMomentumSCSPScalarKernel.h"
...
registerMooseObject("TMAP8App", FlowPathMomentumSCSPScalarKernel);
registerMooseObject("TMAP8App", ADFlowPathMomentumSCSPScalarKernel);

template <bool is_ad>
InputParameters
FlowPathMomentumSCSPScalarKernelTempl<is_ad>::validParams()
{
  ...
  params.addParam<std::vector<MooseFunctorName>>(
      "alphas",
      std::vector<MooseFunctorName>({}),
      "Component flow angles per segment with respect to horizontal (-pi/2 downward to pi/2 upward). Takes a vector of functors.");
  params.addParam<std::vector<MooseFunctorName>>(
      "forms_losses",
      std::vector<MooseFunctorName>({}),
...
template <bool is_ad>
FlowPathMomentumSCSPScalarKernelTempl<is_ad>::FlowPathMomentumSCSPScalarKernelTempl(
  ...
  : Base(parameters),
    ...
    _n_segments(
        this->template getParam<std::vector<MooseFunctorName>>("areas").size()),
    ...
    _gravity(this->template getFunctor<GenericReal<is_ad>>("g"))
{
  ...
  if (_n_segments != area_names.size() || _n_segments != perimeter_names.size() || _n_segments != length_names.size() || 
      _n_segments != alpha_names.size() || _n_segments != forms_loss_names.size() || _n_segments != dPp_names.size() || 
      _n_segments != roughness_names.size() || _n_segments != _n_temps)
  {
    mooseError("Must provide consistent number of segments for each parameter! Including temperatures!");
  }
...
template <bool is_ad>
GenericReal<is_ad>
FlowPathMomentumSCSPScalarKernelTempl<is_ad>::computeQpResidual()
{
  ...
  for (size_t i = 0; i < _n_segments; ++i)
  {
    ...
    momentum_residual += _rhol * _gravity(_qp, _state) * (*(_lengths[i]))(_qp, _state) * sin((*(_alphas[i]))(_qp, _state));
    // Pump pressure
    momentum_residual -= (*(_dPps[i]))(_qp, _state);
    // Transient term + reference pressure drop
    momentum_residual += (*(_lengths[i]))(_qp, _state) / (*(_areas[i]))(_qp, _state) * Base::_u_dot[_i] + _dPc[_i];
  }
...
template <bool is_ad>
Real
FlowPathMomentumSCSPScalarKernelTempl<is_ad>::computeQpJacobian()
{
  if constexpr (!is_ad)
  {
    ...
    for (size_t i = 0; i < _n_segments; ++i)
    {
      ...
      // Friction
      momentum_residual += _fd * (*(_lengths[i]))(_qp, _state) / _Dh * _G / _rhog / (*(_areas[i]))(_qp, _state);
      // Forms
      momentum_residual += (*(_forms_losses[i]))(_qp, _state) * _G / _rhog / (*(_areas[i]))(_qp, _state);
      // Transient term
      momentum_residual += (*(_lengths[i]))(_qp, _state) / (*(_areas[i]))(_qp, _state) * Base::_du_dot_du[_i];
    }
...
template <>
Real
FlowPathMomentumSCSPScalarKernelTempl<true>::computeQpJacobian()
{
  mooseError("Internal error, calling computeQpJacobian in AD class.");
  return 0.0;
}

template class FlowPathMomentumSCSPScalarKernelTempl<false>;
template class FlowPathMomentumSCSPScalarKernelTempl<true>;
