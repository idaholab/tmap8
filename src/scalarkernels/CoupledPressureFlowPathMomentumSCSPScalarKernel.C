...
registerMooseObject("TMAP8App", CoupledPressureFlowPathMomentumSCSPScalarKernel);
registerMooseObject("TMAP8App", ADCoupledPressureFlowPathMomentumSCSPScalarKernel);
...
CoupledPressureFlowPathMomentumSCSPScalarKernelTempl<is_ad>::validParams()
{
  ...
  params.addClassDescription(
      ...
      "This kernel should only be used if the system is a closed loop, and this component should close "
      "one or more FlowPathMomentumSCSPScalarKernels. This is because this component flips the pressure "
      "gradient term sign."
      "Note: Using this kernel also requires using a CoupledODETimeDerivativeScalarKernel, at least until "
...
template <bool is_ad>
CoupledPressureFlowPathMomentumSCSPScalarKernelTempl<is_ad>::CoupledPressureFlowPathMomentumSCSPScalarKernelTempl(
...
CoupledPressureFlowPathMomentumSCSPScalarKernelTempl<is_ad>::computeQpResidual()
{
  ...
  for (size_t i = 0; i < _n_segments; ++i)
  {
    ...
    // Forms
    momentum_residual += (*(_forms_losses[i]))(_qp, _state) * _G * abs(_G) / 2.0 / _rhog * (*(_areas[i]))(_qp, _state) / (*(_lengths[i]))(_qp, _state);
    // Gravity
    // get local density for natural circulation aspect
    auto _rhol = _fp.rho_from_p_T(_Pref(_qp,_state),(*(_T[i]))[_i]);
    momentum_residual += _rhol * _gravity(_qp, _state) * sin((*(_alphas[i]))(_qp, _state)) * (*(_areas[i]))(_qp, _state);
    // Pump pressure
    momentum_residual -= (*(_dPps[i]))(_qp, _state) * (*(_areas[i]))(_qp, _state) / (*(_lengths[i]))(_qp, _state);
