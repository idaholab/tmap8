...
/*                   ALL RIGHTS RESERVED                    */
/************************************************************/
#include "FlowPathEnergySCSPScalarKernel.h"
...
registerMooseObject("TMAP8App", FlowPathEnergySCSPScalarKernel);
registerMooseObject("TMAP8App", ADFlowPathEnergySCSPScalarKernel);

template <bool is_ad>
InputParameters
FlowPathEnergySCSPScalarKernelTempl<is_ad>::validParams()
{
...
template <bool is_ad>
FlowPathEnergySCSPScalarKernelTempl<is_ad>::FlowPathEnergySCSPScalarKernelTempl(
...
template <bool is_ad>
GenericReal<is_ad>
FlowPathEnergySCSPScalarKernelTempl<is_ad>::computeQpResidual()
{
  ...
  auto _mu = _fp.mu_from_p_T(_Pref(_qp,_state),Base::_u[_i]);
  auto _rho = _fp.rho_from_p_T(_Pref(_qp,_state),Base::_u[_i]);
  auto _cp = _fp.cp_from_p_T(_Pref(_qp,_state),Base::_u[_i]);
  auto _k = _fp.k_from_p_T(_Pref(_qp,_state),Base::_u[_i]);
...
template <bool is_ad>
Real
FlowPathEnergySCSPScalarKernelTempl<is_ad>::computeQpJacobian()
{
  if constexpr (!is_ad)
  {
...
template <>
Real
FlowPathEnergySCSPScalarKernelTempl<true>::computeQpJacobian()
{
  mooseError("Internal error, calling computeQpJacobian in AD class.");
  return 0.0;
}

template class FlowPathEnergySCSPScalarKernelTempl<false>;
template class FlowPathEnergySCSPScalarKernelTempl<true>;
